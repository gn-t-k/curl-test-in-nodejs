...
describe("curl from github", () => {
  test("example", async () => {
    const command = "curl https://api.github.com/users/defunkt";
    const expected = {
      login: "defunkt",
      id: 2,
      node_id: "MDQ6VXNlcjI=",
    ...
    const actual = await execCommand(command);

    expect(JSON.parse(actual.stdout)).toStrictEqual(expected);
  });
});
